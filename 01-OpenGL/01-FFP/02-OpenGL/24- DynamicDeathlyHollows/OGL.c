...
float side_length = 0.87f;
float half_perimeter = 0.0f;
float area = 0.0f;
float rotate_T = 0.0f;
float x_t = -3.0f;
float y_t = -3.0f;
...
void draw_triangle();
void draw_inscribed_circle(float , float , float );
void elder_wand();
...
void display(void) {
    ...
    float x1 = 0.0f;
    float y1 = side_length / sqrt(3);
    float x2 = -side_length / 2.0f;
    float y2 = -side_length / (2.0f * sqrt(3));
    float x3 = side_length / 2.0f;
    float y3 = -side_length / (2.0f * sqrt(3));
    ...
    glPushMatrix();
    {   
        glTranslatef(x_t, y_t, -5.0f);
        glRotatef(rotate_T, 0.0f, 1.0f, 0.0f);
        draw_triangle();
    }
    glPopMatrix();
    ...
    glPushMatrix();
    {
        glTranslatef(x_c, y_c, -5.0f);
        glRotatef(rotate_T, 0.0f, 1.0f, 0.0f);
        draw_inscribed_circle(X, Y, radius);
    }
    glPopMatrix();
    
    glPushMatrix();
    {
        glTranslatef(0.0f, y_e, -5.0f);
        // glRotatef(rotate_T, 0.0f, 1.0f, 0.0f);
        elder_wand();
    }
    glPopMatrix();
...
{   
    ...
    glVertex2f(-0.87f, -0.5f);   // Bottom-left vertex
    glVertex2f(0.87f, -0.5f);    // Bottom-right vertex
    glEnd();
}

void draw_inscribed_circle(float X, float Y, float r)
{
    glBegin(GL_LINE_LOOP);
    glColor3f(1.0f, 1.0f, 1.0f);
...
void update(void)
{
    ...
    if (T == 1) {
        x_t += 0.001f;  //movement speed
        y_t += 0.001f;
        if (x_t >= 0.0f && y_t >= 0.0f) {
            T = 0;
            C = 1;
        }
    }
