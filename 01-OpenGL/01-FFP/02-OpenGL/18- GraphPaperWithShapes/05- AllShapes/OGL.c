...
void draw_circle_with_points(float radius);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CHAR:
        ...
        case 'c':
            // draw_circle = TRUE;
            shape = Circle;
            // MessageBox(ghwnd, TEXT("C key pressed!"), TEXT("Key Pressed"), MB_OK | MB_ICONINFORMATION);
            break;
...
void display(void)
{
    ...
    float x_coordinate = 1.0f - distance; 
    ...
    float y_coordinate = 1.0f - distance;
    for (int i = 1; i <= LINE_COUNT / 2; ++i)
    {
        float line_size = 1.0f;
        if (i % 5 == 0)
