...
void resize(width, height)
{
    ...
    GLfloat perspective_near = 0.1f;
    GLfloat perspective_far = 100.0f;
    // GLfloat H = tan(fovy * (0.5f) * M_PI)/(180.0f) * perspective_near;
    GLfloat H = ((tan((fovy/2)/180.0f) * M_PI)) * perspective_near;
    GLfloat W = H * aspect_ratio;
    
    // glFrustum  (l, r,  b, t,n,f);
    // glFrustrum(-W, W, -H, H, znear, zfar);
    perspectiveMatrix[0] = ((2.0f )* perspective_near)/ (W -(-W));
    perspectiveMatrix[1] = 0.0f;
    perspectiveMatrix[2] = 0.0f;
    perspectiveMatrix[3] = 0.0f;

    perspectiveMatrix[4] = 0.0f;
    perspectiveMatrix[5] = ((2.0f) * perspective_near)/(H -(-H));
    perspectiveMatrix[6] = 0.0f;
    perspectiveMatrix[7] = 0.0f;

    perspectiveMatrix[8] = (W + (-W))/(W- (-W)); 
    perspectiveMatrix[9] =(H + (-H))/(H - (-H));
    perspectiveMatrix[10] = -((perspective_far +  perspective_near)/(perspective_far - perspective_near));
    perspectiveMatrix[11] = -1.0f; 
    
    perspectiveMatrix[12] = 0.0f;
    perspectiveMatrix[13] = 0.0f;
    perspectiveMatrix[14] = -(((2.0f)* perspective_far * perspective_near)/ (perspective_far - perspective_near));
