...
void draw_lines_x(float line_size, float y_coordinate);
void draw_lines_y(float line_size, float y_coordinate);
void Concentric_triangle(int );

BOOL Concentric_Triangles = FALSE;
...
void display(void)
{
    ...
    // Calculate the distance between lines based on the number of lines (LINE_COUNT)
    float distance = 2.0f / (LINE_COUNT);

    // Draw lines above the x-axis
    float x_coordinate = 1.0f - distance; // Start below the top line
    for (int i = 1; i <= LINE_COUNT / 2; ++i)
    {
        float line_size = 1.0f;
        if (i % 5 == 0)
            line_size = 2.0f;
        draw_lines_x(line_size, x_coordinate);
        x_coordinate -= distance;
    }

    // Draw lines below the x-axis
    x_coordinate = -1.0f + distance; // Start above the bottom line
    for (int i = 1; i <= LINE_COUNT / 2; ++i)
    {
        float line_size = 1.0f;
        if (i % 5 == 0)
     ...
     float y_coordinate = 1.0f - distance; // Start below the top line
    for (int i = 1; i <= LINE_COUNT / 2; ++i)
    {
        float line_size = 1.0f;
        if (i % 5 == 0)
            line_size = 2.0f;
        draw_lines_y(line_size, y_coordinate);
        y_coordinate -= distance;
    }

    // Draw lines below the x-axis
    y_coordinate = -1.0f + distance; // Start above the bottom line
    for (int i = 1; i <= LINE_COUNT / 2; ++i)
    {
        float line_size = 1.0f;
        if (i % 5 == 0)
    ...
    if(Concentric_Triangles == TRUE)
    {
        Concentric_triangle(10);
    }

    SwapBuffers(ghdc);
}

void draw_lines_x(float line_size, float x_coordinate)
{
    fprintf(gpfile, "x_coordinate = %f\n", x_coordinate);
    glLineWidth(line_size);
    glBegin(GL_LINES);
    glColor3f(0.0f, 0.0f, 1.0f);
    glVertex2f( x_coordinate, -1.0f);
    glVertex2f( x_coordinate, 1.0f);
    glEnd();
}
...
void Concentric_triangle(int no_of_triangles)
{
    ...
    GLfloat step = 1.0f / no_of_triangles;
    ...
    for (int i = 0; i < no_of_triangles; ++i)
    {
        ...
        glVertex3f(-step * (i + 1), -step * (i + 1), 0.0f);
        glVertex3f(step * (i + 1), -step * (i + 1), 0.0f);
        glVertex3f(0.0f, step * (i + 1), 0.0f);
