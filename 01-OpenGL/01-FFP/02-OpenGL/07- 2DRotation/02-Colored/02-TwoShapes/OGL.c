...
void display(void)
{
    ...
    // glTranslatef(3.0f, 0.0f, -6.0f);  //the z co ordinate here will further push the square as there is already z = -6.0f and makes it -6 + -6 = -12.0f 
...
void update(void)
{
    r_angle = r_angle + 1.0f;
    if(r_angle >= 360.0f)
    {
        r_angle = r_angle - 360.0f;
    }

    t_angle = t_angle + 1.0f;
    if(t_angle >= 360.0f)
    {
