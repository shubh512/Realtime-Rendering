...
GLubyte checkImage[checkImageHeight][checkImageWidth][4]; // OpenGL is COLUMN major
                                                          // Therefore height first
...
void loadGLTexture(void)
{
    ...
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1); //1 = Monochrome
   
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT); //for 2D texture S and T is used. 
    //For 3D texture R, S and T is used.
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST); //LINEAR means return the image data as it is 
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    
    //GL_NEAREST  may have reduced quality but processing is fast 
    //GL_LINEAR has high quality but slow processing
